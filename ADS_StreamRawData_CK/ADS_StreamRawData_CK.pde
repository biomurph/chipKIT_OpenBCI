/*
    ChipKit interface to the ADS1299 OpenBCI Shield
    Built with Chip Audette's ADSManager for Arduino renamed ADSManagerCK for the sake of hilarity.
    
*/


//#include <ADSdefinitions.h>   // register addresses and command values
#include <ADS1299ManagerCK.h>
#include <DSPI.h>
DSPI0 ADS;  // DSPI0 is connected to 13,12,11 on UNO32 board

// instantiate the dspiZero!
//DSPI0  ADS;  // DSPI0 is connected to 13,12,11 on UNO32 board best to do this in library?
//ADS.begin(10);  // this should be done in library...

ADS1299ManagerCK ADSManager; //Uses SPI bus and pins to say data is ready.Uses Pins 13,12,11,10,9,8,4
...
//  verbose = true;       // set up for serial feedback
  Serial.begin(115200);
  Serial.println("ADS1299-ChipKIT - Stream Raw Data"); //read the string from Flash to save RAM
  Serial.print("Configured as OpenBCI_Version code = ");Serial.println(OpenBCI_version);
  Serial.flush();
  ...
  Serial.println("Press '?' to query and print ADS1299 register settings again"); //read it straight from flash
  Serial.println("Press 1-8 to disable EEG Channels, q-i to enable (all enabled by default)");
  Serial.println("Press 'F' to enable filters.  'f' to disable filters (disabled by default)");
  Serial.println("Press 'x' (text) or 'b' (binary) to begin streaming data...");     
...
void loop() {
  ...
  if (digitalRead(PIN_STARTBINARY)==LOW) {
    ...
    if (firstReport) { Serial.println("Starting Binary_OpenEEG Based on Pin"); firstReport=false;}
  } else {
    if (startBecauseOfPin) {
      startBecauseOfPin = false;
      stopRunning();
      if (firstReport == false) { Serial.println("Stopping Binary Based on Pin"); firstReport=true;}
    }
  }
    ...
    //if ((sampleCounter % 1) == 0) {
      switch (outputType) {
        case OUTPUT_NOTHING:
          ...
          //if ((sampleCounter % 250) == 1) { Serial.print("Free RAM = "); Serial.println(freeRam()); }; //print memory status
          break;
        case OUTPUT_BINARY:
...
//      stopRunning();
//      Serial.println();
//      Serial.print("Was busy for ");
//      Serial.print(totalMicrosBusy);
//      Serial.println(" microseconds across 250 samples");
//      Serial.print("Assuming a 250Hz Sample Rate, it was busy for ");
//      unsigned long micros_per_250samples = 1000000UL;
//      Serial.print(((float)totalMicrosBusy/(float)micros_per_250samples)*100.0);
//      Serial.println("% of the available time");
//    }
  ...
  while(Serial.available()){      
    ...
    switch (inChar)
    {
      ...
      case 'n':
        ...
        if (is_running) Serial.println("Arduino: Starting, but not outputing to PC...");
        break;
      case 'b':
        toggleRunState(OUTPUT_BINARY);
        startBecauseOfSerial = is_running;
        if (is_running) Serial.println("Arduino: Starting binary...");
        break;
      case 'v':
        toggleRunState(OUTPUT_BINARY_4CHAN);
        startBecauseOfSerial = is_running;
        if (is_running) Serial.println("Arduino: Starting binary 4-chan...");
        break;
     case 's':
        stopRunning();
        startBecauseOfSerial = is_running;
     ...
     case 'x':
        ...
        startBecauseOfSerial = is_running;
        if (is_running) Serial.println("Arduino: Starting text...");
        break;
     case 'f':
        useFilters = false;
        Serial.println("Arduino: disabling filters");
        break;
     case 'F':
        useFilters = true;
        Serial.println("Arduino: enabaling filters");
        break;
     case '?':
        //print state of all registers
