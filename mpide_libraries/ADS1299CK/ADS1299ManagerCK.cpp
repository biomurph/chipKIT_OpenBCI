...
void ADS1299ManagerCK::initialize(void) {
    initialize(OPENBCI_V2);
}

//Initilize the ADS1299 controller...call this once
void ADS1299ManagerCK::initialize(const int version) 
{
	
	
  setVersionOpenBCI(version);
  ADS1299CK::initialize(PIN_DRDY,PIN_RST); // (DRDY pin, RST pin, CS pin, SCK frequency in MHz);
  delay(100);
  ...
  //ADS1299CK::WREG(CONFIG2,0b11010000);delay(1);   //set internal test signal, default amplitude, default speed, datasheet PDF Page 41
  //ADS1299CK::WREG(CONFIG2,0b11010001);delay(1);   //set internal test signal, default amplitude, 2x speed, datasheet PDF Page 41
...
void ADS1299ManagerCK::setVersionOpenBCI(const int version)
{
...
void ADS1299ManagerCK::reset(void)
{
  ADS1299CK::RESET();             // send RESET command to default all registers
  ADS1299CK::SDATAC();            // exit Read Data Continuous mode to communicate with ADS
...
// 
void ADS1299ManagerCK::deactivateChannel(int N)
{
  byte reg, config;
  ...
  ADS1299CK::SDATAC(); delay(1);      // exit Read Data Continuous mode to communicate with ADS

  //shut down the channel
  N = constrain(N-1,0,7);  //subtracts 1 so that we're counting from 0, not 1
  reg = CH1SET+(byte)N;
  config = ADS1299CK::RREG(reg); delay(1);
  ...
  ADS1299CK::WREG(reg,config); delay(1);
  ...
  config = ADS1299CK::RREG(reg); delay(1);//get the current bias settings
  bitClear(config,N);          //clear this channel's bit to remove from bias generation
  ADS1299CK::WREG(reg,config); delay(1);  //send the modified byte back to the ADS
...
void ADS1299ManagerCK::activateChannel(int N,byte gainCode,byte inputCode) 
{
   //check the inputs
  if ((N < 1) || (N > 8)) return;
  ...
  ADS1299CK::WREG(CH1SET+(byte)N,configByte); delay(1);
  ...
  byte biasSettings = ADS1299CK::RREG(reg); //get the current bias settings
  bitSet(biasSettings,N);                   //set this channel's bit
  ADS1299CK::WREG(reg,biasSettings); delay(1);  //send the modified byte back to the ADS
  ...
  ADS1299CK::WREG(CONFIG3,0b11101100); delay(1); 
};

void ADS1299ManagerCK::setSRB1(boolean desired_state) {
	if (desired_state) {
		ADS1299CK::WREG(MISC1,0b00100000); delay(1);  //ADS1299 datasheet, PDF p46
	} else {
		ADS1299CK::WREG(MISC1,0b00000000); delay(1);  //ADS1299 datasheet, PDF p46
	}
}


//Configure the test signals that can be inernally generated by the ADS1299
void ADS1299ManagerCK::configureInternalTestSignal(byte amplitudeCode, byte freqCode)
{
	if (amplitudeCode == ADSTESTSIG_NOCHANGE) amplitudeCode = (ADS1299CK::RREG(CONFIG2) & (0b00000100));
	if (freqCode == ADSTESTSIG_NOCHANGE) freqCode = (ADS1299CK::RREG(CONFIG2) & (0b00000011));
	...
	ADS1299CK::WREG(CONFIG2,message); delay(1);
	
       //ADS1299CK::WREG(CONFIG2,0b11010000);delay(1);   //set internal test signal, default amplitude, default speed, datasheet PDF Page 41
      //ADS1299CK::WREG(CONFIG2,0b11010001);delay(1);   //set internal test signal, default amplitude, 2x speed, datasheet PDF Page 41
      //ADS1299CK::WREG(CONFIG2,0b11010101);delay(1);   //set internal test signal, 2x amplitude, 2x speed, datasheet PDF Page 41
      //ADS1299CK::WREG(CONFIG2,0b11010011); delay(1);  //set internal test signal, default amplitude, at DC, datasheet PDF Page 41
      //ADS1299CK::WREG(CONFIG3,0b01101100); delay(1);  //use internal reference for center of bias creation, datasheet PDF p42 
}
 
//Start continuous data acquisition
void ADS1299ManagerCK::start(void)
{
    ADS1299CK::RDATAC(); delay(1);           // enter Read Data Continuous mode
    ADS1299CK::START();    //start the data acquisition
}
  
//Query to see if data is available from the ADS1299...return TRUE is data is available
int ADS1299ManagerCK::isDataAvailable(void)
{
  return (!(digitalRead(PIN_DRDY)));
}
  
//Stop the continuous data acquisition
void ADS1299ManagerCK::stop(void)
{
    ADS1299CK::STOP(); delay(1);   //start the data acquisition
    ADS1299CK::SDATAC(); delay(1);      // exit Read Data Continuous mode to communicate with ADS
}
...
void ADS1299ManagerCK::printChannelDataAsText(int N, long int sampleNumber)
{
	//check the inputs
	if ((N < 1) || (N > 8)) return;
...
void ADS1299ManagerCK::writeChannelDataAsBinary(int N, long sampleNumber)
{
	//check the inputs
	if ((N < 1) || (N > 8)) return;
...
void ADS1299ManagerCK::writeChannelDataAsOpenEEG_P2(long sampleNumber) {
	ADS1299ManagerCK::writeChannelDataAsOpenEEG_P2(sampleNumber,false);
}
#define synthetic_amplitude_counts (8950)   //counts peak-to-peak...should be 100 uV pk-pk  (100e-6 / (4.5 / 24 / 2^24))
void ADS1299ManagerCK::writeChannelDataAsOpenEEG_P2(long sampleNumber,boolean useSyntheticData) {
	byte sync0 = 0xA5;
	byte sync1 = 0x5A;
	byte version = 2;
...
void ADS1299ManagerCK::printAllRegisters(void)   
{
	boolean prevVerboseState = verbose;
	
        verbose = true;
        ADS1299CK::RREGS(0x00,0x10);     // write the first registers
        delay(100);  //stall to let all that data get read by the PC
        ADS1299CK::RREGS(0x11,0x17-0x11);     // write the rest
        verbose = prevVerboseState;
}

//only use SRB1 if all use_SRB2 are set to false
boolean ADS1299ManagerCK::use_SRB1(void) {
